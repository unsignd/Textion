window.onload = () => {
    ...
    while (fadeList.length > 0) {
        ...
        for (let i = 0; i <= 5000; i++) {
            ...
            if (item.classList.contains('lerpSpeed-' + i))
                speed = i;
        }
...
function Anim(obj, loopTimeout, repeatCount, loopDelay, innerText, lerpSpeed) {
    ...
    setTimeout(() => {
        ...
        changeText = setInterval(() => {
            ...
            if (index === text.length + 1) {
                ...
                opacityLerp(spanText, lerpSpeed);
            }
        }, loopTimeout);
    }, loopDelay);
}

function opacityLerp(obj, lerpSpeed) {
    setTimeout(() => {
        let changeOpacity = setInterval(() => {
            ...
            obj.style.setProperty('opacity', parseFloat(obj.style.opacity) + (1 - parseFloat(obj.style.opacity)) * lerpSpeed);
        }, 10);
    }, 10);
}
